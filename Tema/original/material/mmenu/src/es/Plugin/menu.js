...
class Menu extends Plugin {
  ...
  bindEvents() {
    ...
    this.$el.on('mouseenter.site.menu', '.site-menu-item', function () {
      ...
      .on('touchend.site.menu', '> .site-menu-item > a', function () {
        const $item = $(this).parent('.site-menu-item')
  ...
  expand($item, callback) {
    const self = this
    const $sub = $item.children('.site-menu-sub')
    const $children = $sub.children('.site-menu-item').addClass('is-hidden')
  ...
  position($item, $dropdown) {
    const itemHeight = $item.find('> a').outerHeight()

    const menubarHeight = this.outerHeight
