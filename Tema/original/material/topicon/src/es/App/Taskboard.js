...
class StageList {
  ...
  render() {
    ...
    for (let i = 0; i < length; i++) {
      const stage = this.createStage(this.data[i])
      this.add(stage)
    }
  }
}
...
class Stage {
  ...
  render() {
    ...
    const tasksData = this.data.tasks
  ...
  bindRenameBtn() {
    this.$renameBtn.on('click', function (e) {
      const $header = $(this).closest('.taskboard-stage-header')

      const $menu = $(this).closest('.dropdown-menu')

      const $input = $('.taskboard-stage-rename-input', $menu)

      const $title = $('.taskboard-stage-title', $header)
  ...
  bindRenameSaveBtn() {
    this.$renameSaveBtn.on('click', function () {
      ...
      const $input = $('.taskboard-stage-rename-input', $header)
...
class TaskList {
  ...
  add(task = {}) {
    if (task instanceof Task) {
      ...
      const taskObj = this.createTask(task)
  ...
  render() {
    ...
    for (let i = 0; i < length; i++) {
      const task = this.createTask(this.data[i])
  ...
  bindAddItemToggle() {
    this.$addItemToggle.on('click', () => {
      const $input = $('[name="name"]', this.$wrap)
  ...
  bindTaskInput() {
    this.$el.on('click', '.checkbox-custom input', function (e) {
      ...
      const $target = $this.closest('.list-group-item')

      const taskData = $target.data('taskInfo')
  ...
  handleOpenSlidePanel() {
    const self = this
    const options = $.extend({}, slidePanel.defaults, slidePanel.defaultsOptions)
...
const sildePaneldefaults = Plugin.getDefaults('slidePanel')
const sildePaneldefaultsOptions = {
  template(options) {
    return `
  ...
  afterLoad(object) {
    ...
    const info = $target.data('taskInfo')
  ...
  contentFilter(data, object) {
    ...
    if (info.subtasks.length !== 0) {
      const length = info.subtasks.length
      for (let i = 0; i < length; i++) {
        const $subtask = $(slidePanel.subtaskTpl(info.subtasks[i]))
    ...
    if (info.attachments.length !== 0) {
      const length = info.attachments.length
      for (let i = 0; i < length; i++) {
        const $attachment = $(slidePanel.attachmentTpl(info.attachments[i]))
    ...
    if (info.comments.length !== 0) {
      const length = info.comments.length
      for (let i = 0; i < length; i++) {
        const $comment = $(slidePanel.commentTpl(info.comments[i].src,
...
const slidePanel = {
  ...
  handleSelective($target, selected) {
    ...
    $target.selective({
      ...
      onAfterItemAdd() {
        const $target = this.$element.closest('.slidePanel').data('slidePanel').target
        const arr = getSelected.call(this)

        const taskData = $target.data('taskInfo')
        taskData.members = arr

        $target.data('taskInfo', taskData)
        const $memberList = $target.find('.task-members')
        const memberList = new MemberList($memberList, arr)
      },
  ...
  subtaskTpl(data) {
    const checkedString = data.complete ? 'checked="checked"' : ''
    return `
  ...
  handlePriority() {
    $(document).on('click', '[name="priorities"]', function () {
      const $this = $(this)
      const $target = $this.closest('.slidePanel').data('slidePanel').target
      const taskData = $target.data('taskInfo')
  ...
  handleEditor() {
    $(document).on('click', '.slidePanel .task-title, .taskboard-task-edit, .description-toggle', function () {
      const $this = $(this)
      const $target = $this.closest('.slidePanel').data('slidePanel').target
      const data = $target.data('taskInfo')
  ...
  handleSubtasks() {
    ...
    $(document).on('click', '.subtask-toggle', () => {
      ...
      const $input = $('.subtasks-add .subtask-title')
    ...
    $(document).on('click', '.subtask-add-save', function () {
      ...
      if (value.length === 0) {
        ...
        const $subtask = $(self.subtaskTpl(data))

        $('.subtasks-list').append($subtask)
        taskData.subtasks[length] = data
        $target.data('taskInfo', taskData)
        const $badgeList = $target.find('.task-badges')
        const badgeList = new BadgeList($badgeList, $target.data('taskInfo'))
      }
      $input.val('').focus()
    })
    ...
    $(document).on('click', '.subtask .title', function () {
      ...
      const index = $subtask.index()
      const $input = $('.subtask-title', $subtask)
  ...
  handleDatepicker() {
    $(document).on('click', '.due-date-save', function () {
      ...
      const value = $('#taskDatepickerInput').val()
      if (value.length > 0) {
...
class BadgeList {
  ...
  render() {
    ...
    if (duedateData.length > 0) {
      const duedate = new Duedate(duedateData)
      this.add(duedate)
    }

    if (subtasksData.length > 0) {
      const subtasks = new Subtask(subtasksData)
      this.add(subtasks)
    }

    if (attachmentsData.length > 0) {
      const attachments = new Attachment(attachmentsData)
...
class MemberList {
  ...
  add(member = {}) {
    if (member instanceof Member) {
      ...
      const memberObj = this.createMember(member)
      this.add(memberObj)
    }
  }
...
class AppTaskboard extends Site {
  ...
  bindStageCreateBtn() {
    this.$stageCreateBtn.on('click', () => {
      const $name = $('[name="name"]', this.$model)
      const stageData = {
        title: $name.val(),
        tasks: []
      }
