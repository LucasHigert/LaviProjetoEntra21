...
class AppWork extends BaseApp {
  ...
  handleChart() {
    /* create line chart */
    this.scoreChart = (data) => {
      const scoreChart = new Chartist.Line(data, {
      ...
      scoreChart.on('draw', (data) => {
        if (data.type === 'point') {
          const parent = new Chartist.Svg(data.element._node.parentNode)
          parent.elem('line', {
            x1: data.x,
            y1: data.y,
            x2: data.x + 0.01,
    ...
    /* create bar chart */
    this.barChart = (data) => {
      const barChart = new Chartist.Bar(data, {
  ...
  bindChart() {
    ...
    $(document).on('slidePanel::afterLoad', () => {
      this.scoreChart('.trends-chart')
      this.barChart('.member-chart')
    })
  }
  ...
  handleSelective() {
    ...
    const getNum = (num) => Math.ceil(Math.random() * (num + 1))

    const getMember = () => member[getNum(member.length - 1) - 1]
    ...
    const setItems = (membersNum) => {
      const num = getNum(membersNum - 1)
      pushMember(num)
    }
